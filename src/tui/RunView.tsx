...
export function RunView({ command, cwd, config, verbose, wiki }: RunViewProps): React.ReactElement {
  ...
  return React.createElement(
    ...
    ...events.map((event, i) => React.createElement(EventLine, { key: i, event })),
    running
      ? React.createElement(Text, { color: 'yellow' }, '⏳ Working...')
      : React.createElement(Text, { color: 'green' }, `✓ ${error ? 'Failed: ' + error : 'Done'}`),
    !running ? React.createElement(Text, { color: 'gray' }, 'Press q or Ctrl+C to exit.') : null,
  );
}

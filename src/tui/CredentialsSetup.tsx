...
export function CredentialsSetup({ onConfigSaved }: CredentialsSetupProps): React.ReactElement {
  ...
  async function handleSave(): Promise<void> {
    ...
    try {
      ...
      const globalConfig: GlobalConfig = {
        ...
        ...(effectiveEmbeddingHost !== defaultBaseUrl(mode)
  ...
  if (step === 'api-key') {
    return React.createElement(
      ...
      React.createElement(
        Text,
        { bold: true },
        mode === 'openai' ? 'Enter your API key:' : 'Enter your Ollama Cloud API key:',
      ),
      React.createElement(
        Text,
        { color: 'gray' },
        mode === 'openai'
          ? 'Paste your API key for your OpenAI-compatible endpoint'
  ...
  if (step === 'base-url') {
    return React.createElement(
      ...
      React.createElement(
        Text,
        { color: 'gray' },
        `Press Enter to use the default (${defaultBaseUrl(mode)})`,
      ),
      React.createElement(TextInput, {
        value: baseUrl,
        onChange: setBaseUrl,
  ...
  if (step === 'embedding-model') {
    return React.createElement(
      ...
      React.createElement(
        Text,
        { color: 'gray' },
        `Press Enter to use the default (${DEFAULT_EMBEDDING_MODEL})`,
      ),
      React.createElement(TextInput, {
  ...
  if (step === 'embedding-host') {
    ...
    return React.createElement(
      ...
      React.createElement(
        Text,
        { color: 'gray' },
        `Press Enter to use the default (${defaultHost})`,
      ),
      React.createElement(TextInput, {
        value: embeddingHost,
  ...
  return React.createElement(
    ...
    React.createElement(
      Text,
      { color: 'gray' },
      `Press Enter to use the default (${DEFAULT_LLM_MODEL})`,
    ),
    React.createElement(TextInput, {
      value: model,
      onChange: setModel,

...
const SQLITE_DB_FILES = ['wiki.db', 'wiki.db-journal', 'wiki.db-wal', 'wiki.db-shm'] as const;
 ...
 */
export async function untrackRunMetadataFiles(projectRoot: string): Promise<string[]> {
  const metadataPaths = [
 ...
 * handle both.
 */
export async function runAgent(client: LLMClient, options: RunOptions): Promise<void> {
  const {
    command,
    projectRoot,
    model,
      ...
      if (stream) {
        ...
        for await (const chunk of streamResponse) {
          ...
          if (chunk.message?.tool_calls) {
            toolCalls.push(
              ...chunk.message.tool_calls.flatMap((tc: LLMToolCall) =>
                tc.function?.name
                  ? [
                      {
                        id: tc.id,
        ...
        if (result.message?.tool_calls) {
          toolCalls.push(
            ...result.message!.tool_calls!.flatMap((tc: LLMToolCall) =>
              tc.function?.name
                ? [
                    {
                      id: tc.id,
  ...
  onEvent({
    ...
    result: wikiPublish
      ? 'Created .github/workflows/update-wiki.yml (with wiki publish)'
      : 'Created .github/workflows/update-wiki.yml',
  });
  ...
  // change nothing.
  await writeFile(path.join(projectRoot, '.wiki', '.gitignore'), WIKI_GITIGNORE, 'utf8');
  ...
  await writeFile(path.join(projectRoot, '.wiki', '.last-update-title.txt'), title + '\n', 'utf8');
...
export function generateUpdateReport(
  ...
  if (changedFiles.length === 0) {
    return (
      [`# Wiki ${actionLabel}`, '', 'No files were changed. The wiki is already current.'].join(
        '\n',
      ) + '\n'
    );
  }
  ...
  const lines = [`# Wiki ${actionLabel}`, '', `Run completed at ${timestamp}.`, ''];

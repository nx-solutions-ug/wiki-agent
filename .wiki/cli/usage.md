...
| `GH_TOKEN` | GitHub token for the `gh` CLI tool. Used for the staging PR staleness check and for closing stale `wiki/staging-*` PRs with a comment during update runs. | from environment |

...
| `GH_TOKEN` | GitHub token for the `gh` CLI tool (read-only inspection plus staging PR `close`/`comment` on `wiki/staging-*` branches; used in CI for the staging PR staleness check) | from environment |

...
1. `mode-select` — the user picks `1` for Ollama Local, `2` for Ollama Cloud, or `3` for OpenAI-compatible. The TUI does not accept Enter here; key presses drive transitions.
2. `api-key` — reached from cloud or openai mode. Uses `ink-text-input` to read the key, validates that it is non-empty on submit. The prompt label is provider-specific.

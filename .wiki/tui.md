...
3. `model` — defaults to `kimi-k2.7-code` (matching `config.ts:DEFAULT_MODEL`) and uses the same text input.
...
`RunView` creates the Ollama client via `createOllamaClient(config)` and calls `runAgent` with `stream: true` and `wikiPublish: wiki`. Each `AgentEvent` is translated into a `DisplayEvent` and appended to a ref-backed state list, which Ink re-renders.
...
- `error` — shown in red.
- `done` — shown in green and bold; toggles the "Working…" indicator to "Done" or "Failed: <message>" when the run ended with an error.

...
5. Runs `wiki --update --print --verbose` in headless mode (with `--wiki` appended only when the workflow was generated by `--init --wiki`), with `WIKI_PROVIDER_MODE` taken from the `vars.WIKI_PROVIDER_MODE` repository variable and defaulting to `cloud` (the legacy `WIKI_OLLAMA_API_KEY` secret is also accepted for the API key). The `--verbose` flag makes tool call results appear in the CI log alongside assistant prose. After the run the agent writes `.wiki/.last-update-report.md` and `.wiki/.last-update-title.txt` (when there are changes); `.wiki/.last-updated.json` is a reserved metadata filename only — it is gitignored and excluded from publishing but never written. The publish steps below exist in the template only when `--wiki` was passed to `--init`.
...
9. **Flatten the wiki for GitHub** (only when `--wiki` was passed to `--init` — this and the next step are not in the template otherwise):
...
10. **Publish to wiki repo** (only when `--wiki` was passed to `--init`, content changed, and the wiki is initialized): clones `<repo>.wiki.git` into `/tmp/wiki`, `rsync`s the flattened `/tmp/wiki-flat/` output over the clone excluding `.git`, commits, and **pushes directly to `master`** — the wiki goes live immediately. GitHub wiki repos are hidden Git remotes, not API-accessible repositories, so `gh pr create` cannot open a PR against them; direct push to `master` is the only programmatic publish path. If the push fails, the workflow exits with a `::error::` explaining that the GitHub App needs `contents:write` (which covers the wiki repo) or a `WIKI_PUSH_TOKEN` secret must be set.
11. **Create wiki staging snapshot pull request** (unconditional in the template; only runs when `steps.report.outputs.has_changes == 'true'`): `peter-evans/create-pull-request@v8` adds `.wiki/` on a `wiki/staging-<timestamp>` branch of the main repo and opens a PR titled and described from the run's report. This staging PR is always part of the workflow — it is the audit trail for the `.wiki/` changes even when the wiki-tab publish steps are absent.

...
The workflow sets the `GH_TOKEN` environment variable to the generated GitHub App token or `secrets.GITHUB_TOKEN` so the agent's read-only `gh` CLI staleness check can authenticate.

...
9. **Flatten the wiki for GitHub**:
   ...
   - Metadata files (`.git`, `config.json`, `.last-update-report.md`, `.last-update-title.txt`, `.last-updated.json`, `_plan.md`) are excluded.
...
11. **Create wiki staging snapshot pull request** (only when `steps.report.outputs.has_changes == 'true'`): `peter-evans/create-pull-request@v8` adds `.wiki/` on a `wiki/staging-<timestamp>` branch of the main repo and opens a PR titled with the value from `steps.report.outputs.title` (falling back to `docs: wiki staging snapshot`). This keeps the staged content auditable in the main repo even though the live surface is the wiki tab.
...
The repository also runs automated issue/PR management, OMP-driven workflows, and a lightweight vouch system for PR gating. See [OMP Automation Workflows](./omp.md) for details on `.github/workflows/auto-manage.yml`, `.github/workflows/omp.yml`, `.github/workflows/omp-ci.yml`, `.github/workflows/omp-fix-issue.yml`, and the command prompts under `.omp/commands/`. The vouch system is summarized on the [Development](../development.md) page.

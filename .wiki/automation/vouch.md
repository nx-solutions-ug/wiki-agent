...
last_updated: 2026-09-09T03:46:38.989Z
...
- `vouch-manage.yml` needs `contents: write`, `discussions: write`, and `id-token: write` (required by the App token step).
- `vouch-pr.yml` needs `contents: read`, `pull-requests: write`, `issues: write`, and `id-token: write`.

Both workflows generate a GitHub App token via `actions/create-github-app-token@v3` (`secrets.APP_CLIENT_ID` / `secrets.APP_PRIVATE_KEY`) and use that token for the vouch action; the PR gate also uses it for the `gh label` / `gh pr edit` commands.

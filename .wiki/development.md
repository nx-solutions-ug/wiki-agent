...
- `config.test.ts` — global/project config I/O and `resolveConfig` precedence, including env-var overrides and `0o600` file permissions.
...
- `index-middleware.test.ts` — `index.md` regeneration, exclusions, idempotency, and frontmatter parsing edge cases.
...
Produces a tarball named from `package.json` `version` (for example `wiki-agent-1.10.0.tgz`). The tarball includes `dist/`, `README.md`, and `LICENSE` per the `files` array in `package.json`.
...
- `.github/workflows/omp-ci.yml` — automated OMP triage, PR labeling, and PR review triggered by issues/PR events; also runs the `fix-issue` follow-up after triage.
- `.omp/commands/*.md` — parameterized OMP prompts (triage, label, review, fix).
- `.omp/agent/config.yml` — OMP model mapping (`default`, `task`, `commit` → `ollama-cloud/minimax-m3`).
...
The package is published to npm as `@chronova/wiki-agent`. The GitHub Actions workflow uses `npx --yes semantic-release` with plugins from `@semantic-release/*` to version, tag, write release notes, and publish. The tarball produced by `bun pm pack` (or `npm pack`) includes `dist/`, `README.md`, and `LICENSE` only; it does not ship workflow files, so the exact `.github/workflows/*.yml` filenames do not affect the published package.

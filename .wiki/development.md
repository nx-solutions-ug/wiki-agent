...
- `.github/workflows/omp.yml` — invokes the OMP agent on comments containing `/omp` (a leading `/oc` alias is stripped during prompt extraction, but only `/omp` comments trigger the run) and routes command prompts from `.omp/commands/*.md` into OMP.

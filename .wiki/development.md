...
- Node.js 22+ (declared in `package.json` `engines.node`), though the CI workflows currently use Node.js 25. Node runs the compiled CLI.
...
Produces a tarball named like `wiki-agent-<version>.tgz` (for example `wiki-agent-1.6.3.tgz`). The tarball includes `dist/` and `README.md` only.

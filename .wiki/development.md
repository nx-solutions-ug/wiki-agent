...
- `tools.test.ts` — path-safety checks, file read/write/edit, tool definition shape, `git` and `gh` subcommand allowlists, metacharacter guard, `ast_grep`/`ast_search` structural matching, and `parseArgsStringToArgv`.

...
All write operations are constrained to `.wiki/`. `resolveWikiPath` in `tools.ts` rejects any path whose absolute resolution escapes the `.wiki/` directory. Read-only tools (`read_file`, `ls`, `grep`, `glob`, `git`, `ast_grep`, `ast_search`, `gh`) are constrained to the project root. The `git` tool is limited to a read-only subcommand allowlist and rejects shell metacharacters; the `gh` tool is limited to read-only inspection plus `pr close`/`pr comment` only on `wiki/staging-*` PRs. The `grep`, `glob`, `git`, and `gh` tools invoke external binaries through `execFileAsync` (array arguments, no shell), which prevents command injection through model-controlled input. The old general-purpose `execute` shell tool has been removed; `ast_grep` and `ast_search` still use `execAsync` with single-quote escaping and are therefore less hardened.

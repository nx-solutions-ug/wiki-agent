...
This step is invoked by `.github/workflows/update-wiki.yml` (when `--wiki` was passed to `--init`) immediately before the wiki repo is cloned and rsynced. See [GitHub Actions](../automation/github-actions.md). The workflow file itself is generated by `createWorkflowFile` in `src/agent.ts`; recent fixes added `actions/setup-node@v7` (required by the `wiki-flatten` binary) and `GH_TOKEN` (so the agent's `gh` CLI calls are authenticated in CI).

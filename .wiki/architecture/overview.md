...
- `agent.ts` — the agent loop, provider-agnostic tool calling, event stream
- `config.ts` — global/project config, provider-agnostic LLM client construction
...
2. Construct the provider `chat` request with the current `messages` array and the tool definitions (Ollama SDK or OpenAI SDK via a common adapter interface).
...
5. Append the assistant message to the history. If there are tool calls, append a `tool` message per call. Ollama associates results with `tool_name`, while OpenAI uses `tool_call_id`; the runtime includes both so either adapter can associate the result. Successful `write_file`/`edit_file` calls also record a per-file description from the assistant's preceding prose (falling back to the tool result) for the update report; this description and the final report are both passed through `stripThinkingTags` so reasoning blocks do not leak into PR bodies.
...
Errors from the underlying SDK are surfaced through the `error` event stream. If the model had already produced content, the loop exits with a `done` summary that includes the error message; otherwise it emits `error` and stops.
...
- Headless mode (`--print`) sets `stream: false` and writes assistant content (wrapped in blank lines), tool results (only with `--verbose`), and the final summary to stdout/stderr. The same loop works with either the Ollama or OpenAI-compatible adapter.
...
- If `config.mode === "cloud"` or `config.mode === "openai"` and no API key is present, `App` renders `CredentialsSetup` first. The user selects local, cloud, or openai, enters the API key (cloud/openai only), and a model ID. The result is persisted to `~/.wiki/config.json` and re-resolved.

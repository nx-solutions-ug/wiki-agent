...
1. Generates a GitHub App token if `APP_ID` and `APP_PRIVATE_KEY` secrets are set (falls back to `GITHUB_TOKEN`)
...
| `APP_ID` | No | GitHub App ID for token generation (falls back to `GITHUB_TOKEN`) |

...
describe('OpenAIAdapter', () => {
  it('streams chunks and accumulates tool calls', async () => {
    const stream = async function* () {
      yield {
        choices: [
          {
            delta: { tool_calls: [{ index: 0, id: 'call_123', function: { name: 'test_tool' } }] },
          },
        ],
      };
      yield {
    ...
    expect(chunks[1]).toEqual({
      message: {
        content: '',
        tool_calls: [{ id: 'call_123', function: { name: 'test_tool', arguments: { arg: 1 } } }],
      },
    });
  });
  ...
  it('streams chunks where name arrives later', async () => {
    const stream = async function* () {
      ...
      yield {
        choices: [{ delta: { tool_calls: [{ index: 0, function: { name: 'delayed_tool' } }] } }],
      };
      yield {
        choices: [{ delta: { tool_calls: [{ index: 0, function: { arguments: '{"arg": 2}' } }] } }],
      };
    };
    ...
    expect(chunks[0]).toEqual({
      message: {
        content: '',
        tool_calls: [{ id: 'call_456', function: { name: 'delayed_tool', arguments: { arg: 2 } } }],
      },
    });
  });

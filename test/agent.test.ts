...
describe('runAgent', () => {
  ...
  test('runs agent loop without tools and exits', async () => {
    ...
    expect(events).toContainEqual({
      type: 'done',
      summary: 'Wiki is already current. No files changed.',
    });
  });
  ...
  test('executes tool calls and loops', async () => {
    ...
    const toolEvent = events.find(
      (e) => e.type === 'tool' && e.name === 'read_file' && e.result !== '',
    ) as { type: 'tool'; name: string; result: string };
  ...
  test('generates report and creates PR metadata for changed files', async () => {
    ...
    await expect(
      readFile(path.join(projectRoot, '.wiki', '.last-updated.json'), 'utf8'),
    ).rejects.toThrow();

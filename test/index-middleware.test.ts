...
async function writeWikiFile(root: string, relativePath: string, content: string): Promise<void> {
...
describe('index-middleware', () => {
  ...
  test('indexes subdirectories', async () => {
    ...
    const subIndex = await readFile(path.join(wikiRoot, 'architecture', 'index.md'), 'utf8');
  ...
  test('index entries are deterministically sorted and complete across chunk boundaries', async () => {
    ...
    const expectedNames = Array.from({ length: FILE_COUNT }, (_, i) => makeName(i)).sort((a, b) =>
      a.localeCompare(b),
    );
  ...
  test('rejects when a file has invalid frontmatter instead of silently dropping it', async () => {
    await writeWikiFile(wikiRoot, 'good.md', '---\ntitle: Good\n---\n# Good\n');
    await writeWikiFile(
      wikiRoot,
      'bad.md',

...
describe('generateUpdateTitle', () => {
  ...
  test('only created pages are reflected', () => {
    expect(
      generateUpdateTitle('update', [{ action: 'created', path: '.wiki/a.md', description: '' }]),
  ...
  test('only edited pages are reflected', () => {
    expect(
      generateUpdateTitle('update', [{ action: 'edited', path: '.wiki/a.md', description: '' }]),
  ...
  test('init with changes uses initialize label', () => {
    expect(
      generateUpdateTitle('init', [{ action: 'created', path: '.wiki/a.md', description: '' }]),
...
describe('filterReportFiles', () => {
  test('excludes run metadata files', () => {
    const input = [
      ...
      {
        action: 'created',
        path: '.wiki/.last-updated.json',
        description: 'Wrote .wiki/.last-updated.json',
      },
      {
        action: 'created',
        path: '.wiki/.last-update-report.md',
        description: 'Wrote .wiki/.last-update-report.md',
      },
      {
        action: 'created',
        path: '.wiki/.last-update-title.txt',
        description: 'Wrote .wiki/.last-update-title.txt',
      },
    ];
...
describe('untrackRunMetadataFiles', () => {
  ...
  test('untracks metadata files when tracked in git', async () => {
    ...
    const { stdout } = await execFileAsync('git', ['ls-files', '.wiki/.last-updated.json'], {
      cwd: projectRoot,
    });

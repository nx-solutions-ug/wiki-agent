...
describe('tools', () => {
  ...
  describe('path safety', () => {
    ...
    test('write_file accepts paths under .wiki/ and adds frontmatter', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'quickstart.md'), 'utf8');
  ...
  describe('read_file', () => {
    test('reads file content', async () => {
      ...
      const result = await executeTool('read_file', { path: 'test.txt' }, projectRoot);
  ...
  describe('edit_file', () => {
    test('replaces text in existing file', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'test.md'), 'utf8');
  ...
  describe('thinking tag stripping', () => {
    ...
    test('write_file strips thinking blocks', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'arch.md'), 'utf8');
    ...
    test('write_file strips reasoning and reflection blocks', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'cli.md'), 'utf8');
    ...
    test('write_file strips thinking tags spanning multiple lines', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'multi.md'), 'utf8');
      expect(content).not.toContain(`${LT}think`);
    ...
    test('write_file leaves content without thinking tags unchanged', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'clean.md'), 'utf8');
      expect(content).toContain('type: Guide');
    ...
    test('edit_file strips thinking tags from new_string', async () => {
      ...
      const content = await readFile(path.join(projectRoot, '.wiki', 'edit.md'), 'utf8');
      expect(content).not.toContain(`${LT}think`);
  ...
  describe('git tool', () => {
    ...
    test('rejects non-git subcommands', async () => {
      const result = await executeTool('git', { args: 'rm -rf .' }, projectRoot);
      expect(result).toContain('not permitted');
    });

    test('rejects mutating subcommands', async () => {
      const result = await executeTool('git', { args: 'commit -m test' }, projectRoot);
      expect(result).toContain('not permitted');
    });
    ...
    test('rejects shell metacharacters', async () => {
      const result = await executeTool('git', { args: 'log --oneline; rm -rf .' }, projectRoot);
      expect(result).toContain('metacharacters');
    });

    test('allows read-only log in a git repo', async () => {
      const result = await executeTool('git', { args: 'log --oneline' }, projectRoot);
  ...
  describe('ast_search tool', () => {
    test('runs an inline yaml rule', async () => {
      ...
      const result = await executeTool('ast_search', { rule, path: 'src' }, projectRoot);
  ...
  describe('gh tool', () => {
    test('rejects non-gh subcommands', async () => {
      const result = await executeTool('gh', { args: 'auth login' }, projectRoot);
      expect(result).toContain('not permitted');
    });

    test('rejects pr create', async () => {
      const result = await executeTool('gh', { args: 'pr create --title test' }, projectRoot);
      expect(result).toContain('blocked operation');
    });

    test('rejects pr merge', async () => {
      const result = await executeTool('gh', { args: 'pr merge 123' }, projectRoot);
      expect(result).toContain('blocked operation');
    });

    test('rejects pr close without a valid PR number', async () => {
      const result = await executeTool('gh', { args: 'pr close abc' }, projectRoot);
      expect(result).toContain('valid PR number');
    });

    test('rejects pr close when PR verification fails (no gh auth)', async () => {
      const result = await executeTool('gh', { args: 'pr close 999' }, projectRoot);
    ...
    test('rejects pr comment on non-staging branch', async () => {
      ...
      const result = await executeTool('gh', { args: 'pr comment 999 --body test' }, projectRoot);
      expect(result).toContain('Error');
    });
    ...
    test('rejects shell metacharacters', async () => {
      const result = await executeTool('gh', { args: 'pr list; rm -rf .' }, projectRoot);
      expect(result).toContain('metacharacters');
    });

    test('rejects issue create', async () => {
      const result = await executeTool('gh', { args: 'issue create --title test' }, projectRoot);
      expect(result).toContain('blocked operation');
    });

    test('rejects run rerun', async () => {
      const result = await executeTool('gh', { args: 'run rerun 123' }, projectRoot);
      expect(result).toContain('blocked operation');
    });
  });
  ...
  describe('grep tool', () => {
    test('prevents command injection via pattern', async () => {
      ...
      const result = await executeTool('grep', { pattern: "'; echo 'vulnerable" }, projectRoot);
      expect(result).not.toContain('vulnerable');
    });
    ...
    test('searches for a pattern and returns matches', async () => {
      ...
      const result = await executeTool('grep', { pattern: 'hello' }, projectRoot);
  ...
  describe('glob tool', () => {
    test('prevents command injection', async () => {
      const result = await executeTool('glob', { pattern: "'; echo 'vulnerable" }, projectRoot);
      expect(result).not.toContain('vulnerable');
    });

    test('finds files matching a wildcard pattern', async () => {
      await writeFile(path.join(projectRoot, 'test.md'), '# Test\n');
      const result = await executeTool('glob', { pattern: '*.md' }, projectRoot);
    ...
    test('normalizes ** prefix so recursive patterns work with find', async () => {
      ...
      const result = await executeTool('glob', { pattern: '**/*.ts' }, projectRoot);

...
## 2026-07-31 - File Reading Overhead for Frontmatter
**Learning:** Loading the entire contents of a file into a single string in memory via `readFile` just to parse the top-level frontmatter creates a performance bottleneck on large markdown files. This bloats memory usage unnecessarily and places significant pressure on garbage collection.
**Action:** When extracting top-level frontmatter from files, use a chunked reading approach via `fs.promises.open` and `fd.read`, with `node:string_decoder`, rather than `fs.readFile` to minimize memory bloat.
